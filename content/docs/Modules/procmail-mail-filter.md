...
When installed on a system, Procmail effectively replaces the normal mail.local email delivery command that [Sendmail Mail Server](/docs/modules/sendmail-mail-server) and other MTAs run to append a message to a user's mail file. Even though it is most commonly used with Sendmail, the other MTA [Postfix Mail Server](/docs/modules/postfix-mail-server) can be configured to use Procmail for delivery as well. As far as the program is concerned, the actual mail server in use does not matter as long as email is passed to it properly. 
...
Procmail is most useful when configured by individual users to perform tasks such as sorting email from different people into different mailboxes, writing to two different mail files or dropping email from specific addresses. The Procmail Webmin module and this chapter only deal with system-wide configuration though, if you want a tool that lets individual users configure the program though a web interface, [Usermin](/docs/usermin) is the program to use. It has a module with an identical interface that manages `.procmailrc` files instead of `/etc/procmailrc`, and another with an even simpler interface.
...
[![](/images/docs/screenshots/modules/light/procmail-mail-filter.png "Procmail Mail Filter Screenshot")](/images/docs/screenshots/modules/light/procmail-mail-filter.png)

Unlike other modules, this one will not complain if Procmail is not installed on your system. You should use the [Software Packages](/docs/modules/software-packages) module to check for and install the package that comes with your Linux distribution or operating system.
...
- Go to the [Sendmail Mail Server](/docs/modules/sendmail-mail-server) module, which can be found in Webmin under the Servers category. 
...
- Select the type of destination for messages that match this action from the **Delivery mode** menu. The available options are:
  - **Append to file** &mdash; Email will be appended in standard mailbox format to the file entered in the adjacent text field, such as `/var/spool/mail/fred`. To throw a message away, enter `/dev/null` as the file. 
  - **Write to maildir** &mdash; Matching email will be added to the Qmail-style mail directory whose path is entered in the text field. If this directory does not exist yet, Procmail will create it (and the needed subdirectories) for the user.
  - **Write to MH folder** &mdash; Email will be added to the specified MH-style mail directory. This mail format also uses one file per message, but places them in all a single directory and gives message files incrementing numeric filenames, like _1_, _2_, _3_ and so on.
  - **Forward to address** &mdash; Email will be sent to the address or user entered in the adjacent text field, such as *foo@example.com*.
  - **Feed to program** &mdash; Email messages that match will be fed as input to the program whose path and arguments are entered into the text box next to the menu. If a non-absolute mail filename or directory (like *Mailbox* or *Maildir*) is entered, Procmail will assume that it is relative to the home directory of the user to whom the email is being delivered. 
...
- The **Action conditions** section of the form is for entering the conditions that determine which messages will be delivered by this action. If none are specified, messages that reach the action will always be delivered, and if more than one is entered they must all match for delivery to take place. This section is actually a table that starts out with two blank rows. The menu in each row determines the type of condition and how the text in the box next to it is interpreted. The available options are:
  - **Matches regular expression** &mdash; For this condition to match the message headers (and possibly the body too) must match the Perl-style regular expression entered in the text box. Remember that this expression is apply to all the headers as though they were a single block of text, so you should precede any header name with a ^ to indicate the start of a line. For example, to catch messages whose subject contains the word `foo` you could enter `^Subject:.*foo.*`.
  - **Doesn't match regular expression** &mdash; This condition type works just like the previous one, except that it matches messages that do not match the regular expression.
  - **Evaluate output of command** &mdash; The shell command entered in the text box will be run, its output read by Procmail and then interpreted again as an action line from the configuration file. This type of condition is extremely powerful as it allows you to create dynamically generated conditions - however, for everyday mail filtering you probably don't need to use it.
  - **Check exit status of command** &mdash; This type of condition matches if the shell command entered has an exit status of zero, indicating success. It can be used to have mail delivered to different destinations depending on the system's hostname, the time of day or the existence of some file.
  - **Mail is smaller than** &mdash; The condition will match if the total size of the message is smaller than the number of bytes entered in the adjacent text box.
  - **Mail is bigger than** &mdash; As its name suggests, this type of condition is the opposite of the previous one. 
...
[![](/images/docs/screenshots/modules/light/procmail-mail-filter-create-action.png "Procmail Mail Filter Create Action Screenshot")](/images/docs/screenshots/modules/light/procmail-mail-filter-create-action.png)
...
### See also
* Filtering spam with [SpamAssassin Mail Filter](/docs/modules/spamassassin-mail-filter)
